...
class ParcelUpdate():
    ...
    def parcelUpdateGUI(self, title, selec):
        ...
        self.sellerFrame = LabelFrame(self.root, bg=bg_color, padx=5, labelanchor='n', width=100, height=400,
                                      text='신청한 사용자')
        ...
        # button 설정
        btn_update = Button(self.root, width=10, text='수정', bg='#F0AD48', command=self.updateEvent, relief='flat',
                            bd=10, fg='#B96F00')
        btn_back = Button(self.root, width=10, text='삭제', bg='#F0AD48', command=self.deleteEvent, relief='flat', bd=10,
                          fg='#B96F00')
    ...
    def draw_info(self, list, field, selec):  # gui에 저장된 게시물 미리 세팅하기 (entry리스트, [종류튜플, 성별튜플], 선택한 동물)
        ...
        for i, d in enumerate(apply_list):
            btn = Button(self.sellerFrame, padx=13, text=d, pady=5, fg='#B96F00', bg='#F0AD48', command=lambda x=i: self.sellerEvent(d))
